...
import './AlumnosAlova.css';

function AlumnosAlova() {
  ...
  const fetchAlumnos = async () => {
    try {
      ...
      const data = await getAlumnos.send();
      setAlumnos(data);
      setError(null);
    } catch (err) {
  ...
  const fetchCursos = async () => {
    try {
      ...
      const data = await getCursos.send();
      setCursos(data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const postAlumno = alovaInstance.Post('/alumnos', nuevoAlumno);
      await postAlumno.send();
